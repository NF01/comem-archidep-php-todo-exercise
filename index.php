...
if (isset($_POST['action'])) {
  switch($_POST['action']) {
    ...
    case 'new':
      ...
      $title = $_POST['title'];
      if ($title && $title !== '') {
        $insertQuery = $db->prepare('INSERT INTO todo VALUES(NULL, ?, FALSE, CURRENT_TIMESTAMP)');
       if (!$insertQuery->execute(array($title))) {
       die(print_r($insertQuery->errorInfo(), true));
        }
      }
      ...
      <div class="navbar navbar-dark bg-dark shadow-sm">
        <div class="container d-flex justify-content-between">
          <a href="#" class="navbar-brand d-flex align-items-center">
            <strong>A (wonderful) TodoList</strong>
          </a>
        </div>
      </div>
    </header>
